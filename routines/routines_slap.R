...
################################################################################
# Minimum Discrepancy measure: normal(mu,sigma) vs skew Laplace
# lambda: real number, skewness parameter
################################################################################
# lambda: real parameter

discrepancy_min_slap <- Vectorize(function(lambda){
  ...
  if(lambda != 0){
    ...
    disc <- function(par){
      ...
      tempf <- Vectorize(function(x){
        ...
        den <- dlap(x,par[1],exp(par[2])) + 2*dlap(x)*plap(lambda*x)
        out <- num/den
        return(out)
      })
...
sdiscrepancy_min_slap <- Vectorize(function(lambda){
  if(lambda ==0 ){ 
        sgn = 0
        val = 0
  }
...
unprior_min_slap <- Vectorize(function(lambda){
  if(lambda ==0 ) prior = 0
  if(lambda != 0){
    # Absolute value using the symmetry of the discrepancy measure
    lambda <- abs(lambda)
    ...
    tempf2 <- Vectorize(function(x){
      num <- -(dlap(x,mu_opt,sd_opt)^2)*2*dlap(x)*dlap(lambda*x)*x
      den <- (dlap(x,mu_opt,sd_opt) + 2*dlap(x)*plap(lambda*x))^2
      out <- num/den
      return(out)
    })
    # prior
